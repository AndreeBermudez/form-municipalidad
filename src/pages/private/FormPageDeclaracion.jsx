...
const FormPageDeclaracion = () => {
  ...
  // Extraer el tipo de persona; se asume "juridica" si no se especifica
  const personType = location.state?.personType || 'juridica';

  // Definir los pasos de forma condicional:
  // - Para "juridica": 6 pasos (incluye "Representante")
  // - Para "natural": 5 pasos (omitiendo "Representante")
  const steps =
    personType === 'juridica'
  ...
  const currentStep = personType === 'juridica' ? 6 : 5;
  ...
  const handleBack = () => navigate('/formulario/pag-ubicacion', { state: { personType } });
  // Redirige al resumen final
  const handleNext = () => navigate('/formulario/resumen', { state: { personType } });
